...
class PiPong:
        ...
        def __init__(self):
                ...
                if 2 == pygame.joystick.get_count ():
                        self.player1Joy = pygame.joystick.Joystick(0)
                        self.player1Joy.init ()
                        self.player2Joy = pygame.joystick.Joystick(1)
                        self.player2Joy.init ()
        ...
        def handleEvents(self):
                ...
                for event in pygame.event.get():
                        ...
                        if event.type == JOYAXISMOTION:
                                if self.player1Joy.get_axis (1) > 0.1:
                                        # Start moving bat
                                        self.player1Bat.startMove("down")
                                elif self.player1Joy.get_axis (1) < -0.1:
                                        self.player1Bat.startMove("up")
                                else:
                                        self.player1Bat.stopMove()

                                if self.player2Joy.get_axis (1) > 0.1:
                                        self.player2Bat.startMove("down")
                                elif self.player2Joy.get_axis (1) < -0.1:
...
class Bat(sprite.Sprite):
        ...
        def update(self):
                ...
                if self.moving:
                        ...
                        if self.direction == "up" and self.rect.centery > 0:
                                self.rect.centery -= self.speed
                        elif self.direction == "down" and self.rect.centery < self.displaySize[1]:
                                self.rect.centery += self.speed
